...
import syrto
sys.modules['inverse_log'] = syrto.utils # For retro-compatibility when loading checkpoint. Not used for classic baseline models
...
def read(data_file, data_dir, min_cutoff={"Turnover":1e4}, max_cutoff={"Turnover":1e8}):
    ...
    '''
    df = syrto.utils.read_dataset(data_dir, data_file, logspace=False, min_cutoff={"Turnover":1e4}, max_cutoff={"Turnover":1e8})
    if sector_level is not None:
        df = df[df[sector_level].isin(sector)] 
    return df


#Current targets for multivariate analisys. In case of baseline models also correspond to data features
targets = ["FixedAssets",
           "CurrAssets",
           "Debtors",
           "Cash",
...
df = read('data_full_1.3.parquet', './dataset')
